...
public class ClubMemberServiceImpl implements ClubMemberService {
	...
	public void handleJoinRequest(Long clubId, Long joinRequestId, JoinStatus status, AuthUser approver) {
		JoinRequest jr = joinRequestRepository
			.joinRequestByIdAndClubId(joinRequestId, clubId)
		...
		if (status == JoinStatus.APPROVED) {
			Optional<ClubMember> existing = clubMemberRepository
				.findByClubAndUserWithAnyStatus(club, user);
			...
			if (existing.isPresent()) {
				...
				ClubMember nm = new ClubMember(
					club, user, ClubMemberRole.MEMBER, ClubMemberStatus.ACTIVE
				);
	...
	public void changeMemberRole(
		Long clubId,
		AuthUser authUser, Long targetMemberId,
		...
		ClubMember operatorMember = clubMemberRepository
			.findByClubIdAndUserId(clubId, operatorUserId)
			.orElseThrow(() -> new BusinessException(
				ResponseCode.INVALID_REQUEST, "권한이 없습니다."
			));
		...
		if (newRole == ClubMemberRole.OWNER) {
			...
			ClubMember currentOwner = clubMemberRepository
				.findOwnerByClub(club)
			...
			if (operatorMember.getClubMemberRole() == ClubMemberRole.ADMIN
	...
	public void withdrawFromClub(Long clubId, AuthUser user) {
		ClubMember me = clubMemberRepository
			.findByClubIdAndUserId(clubId, user.id())
	...
	public void kickMember(Long clubId, AuthUser operator, Long targetMemberId) {
		ClubMember op = clubMemberRepository
			.findByClubIdAndUserId(clubId, operator.id())
			.orElseThrow(() -> new BusinessException(ResponseCode.INVALID_REQUEST, "권한이 없습니다."));
		if (op.getClubMemberRole() != ClubMemberRole.OWNER
			&& op.getClubMemberRole() != ClubMemberRole.ADMIN) {
	...
	public Page<ClubMemberInfoResponseDto> listMembers(
		Long clubId,
		AuthUser authUser,
		Pageable pageable
	) {
		...
		boolean isMember = clubMemberRepository
			.findByClubIdAndUserId(clubId, authUser.id())
		...
		return page.map(cm -> new ClubMemberInfoResponseDto(
			cm.getId(),
			cm.getUser().getId(),
			cm.getUser().getName(),
