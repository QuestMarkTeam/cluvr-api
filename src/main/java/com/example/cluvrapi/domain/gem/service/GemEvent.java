...
import com.example.cluvrapi.domain.gem.listener.dto.GemEventRequestDto;
...
public class GemEvent extends UserEvent<GemEventRequestDto> {
	public GemEvent(Long userId, RedisKey redisKey, UserEventType type, GemEventRequestDto dto) {
	...
	public static GemEvent createEvent(Long userId, Integer gem, String description, LocalDateTime createdTime,
		...
		return new GemEvent(userId, RedisKey.GEM_LOG, UserEventType.GEM,
			GemEventRequestDto.of(userId, gem, description, createdTime, deletedTime, flowType, action)
		);
	}
}
